...
export class MusicController {
  ...
  async support(@Req() req: any) {
    const id = (req.params && req.params.id) as string;
    const res = await this.musicService.initiateSupport(
      req.user,
      id,
      req.body || {},
    );
  ...
  @Post(":id/play")
  async trackPlay(@Req() req: any, @Param("id") musicId: string) {
    // Frontend should hit this endpoint when a song plays for > 5 seconds
    const data = await this.musicService.logPlayHistory(req.user, musicId);
  ...
  async getMyHistory(
    ...
    const data = await this.musicService.getUserHistory(req.user, limit);
    return { success: true, data };
  }

  // --- Queue Endpoints ---
  @Post("me/queue")
  async updateQueue(@Req() req: any, @Body("trackIds") trackIds: string[]) {
    // Frontend sends an array of Music IDs to schedule what plays next
    if (!Array.isArray(trackIds)) {
      throw new BadRequestException("trackIds must be an array");
    }
    const data = await this.musicService.syncQueue(req.user, trackIds);
    return { success: true, data };
  }

  @Get("me/queue")
  async getMyQueue(@Req() req: any) {
    const data = await this.musicService.getQueue(req.user);
    return { success: true, data };
  }
}
