...
export class UserService extends BaseService<IUser> {
  ...
  async searchWithFilters(
    filters: UserSearchFilterDto,
    excludeUserIds: string[],
    currentUser: IUser
  ) {
    const query: any = {};

    // 1. EXCLUDE BANNED USERS AND CURRENT USER
    query._id = { $nin: excludeUserIds.map((id) => new Types.ObjectId(id)) };
    ...
    // 7. NAME SEARCH FILTER
    if (filters.search) {
      query.$or = [
        { fullName: new RegExp(filters.search, "i") },
        { fullNameEn: new RegExp(filters.search, "i") },
  ...
  async findOneByEmailOrThrow(email: string, select: string) {
    ...
    if (!usr) throw new NotFoundException(i18nApi.userEmailNotFound);
    return usr;
  }
  ...
  findByIdForBalance(id: string, select?: string): Promise<IUser | null> {
    console.log(
      "UserService: findByIdForBalance called with id:",
      id,
      "type:",
      typeof id
    );
  ...
  async findByIdOrThrow(
    ...
    if (!user)
      throw new NotFoundException("User with id " + id + " Not exist in db");
    return user;
  }
  ...
  async searchV2(dto: Object, notContains: any[]) {
    ...
    paginationParameters[1].select =
      "fullName fullNameEn userImage bio phoneNumber createdAt isPrime roles hasBadge";
  ...
  async addToBalance(userId: string, amount: number): Promise<IUser | null> {
    console.log(
      "UserService: addToBalance called with userId:",
      userId,
      "amount:",
      amount
    );
  ...
  async subtractFromBalance(
    ...
    console.log(
      "UserService: subtractFromBalance called with userId:",
      userId,
      "amount:",
      amount
    );
  ...
  async setBalance(userId: string, amount: number): Promise<IUser | null> {
    console.log(
      "UserService: setBalance called with userId:",
      userId,
      "amount:",
      amount
    );
    const objectId = new mongoose.Types.ObjectId(userId);
    return this.model
      .findOneAndUpdate({ _id: objectId }, { balance: amount }, { new: true })
      .lean();
  }

  // Claimed gifts management methods
  async addClaimedGift(
    userId: string,
    giftMessageId: string
  ): Promise<IUser | null> {
    console.log(
      "UserService: addClaimedGift called with userId:",
      userId,
      "giftMessageId:",
      giftMessageId
    );
  ...
  async isGiftClaimed(userId: string, giftMessageId: string): Promise<boolean> {
    console.log(
      "UserService: isGiftClaimed called with userId:",
      userId,
      "giftMessageId:",
      giftMessageId
    );
    ...
    const user = await this.model
      .findOne({ _id: objectId }, "claimedGifts")
      .lean();
    return user ? user.claimedGifts.includes(giftMessageId) : false;
  }

  async removeClaimedGift(
    userId: string,
    giftMessageId: string
  ): Promise<IUser | null> {
    console.log(
      "UserService: removeClaimedGift called with userId:",
      userId,
      "giftMessageId:",
      giftMessageId
    );
  ...
  async getUserBalance(userId: string): Promise<number> {
    console.log(
      "UserService: getUserBalance called with userId:",
      userId,
      "type:",
      typeof userId
    );
  ...
  async getUserBalanceDirect(userId: string): Promise<number> {
    console.log(
      "UserService: getUserBalanceDirect called with userId:",
      userId,
      "type:",
      typeof userId
    );
