...
export class ChannelService {
  ...
  async getAllRoomsForUser(userId: string) {
    ...
    const memberships = await this.roomMemberService.findAll({ uId: userId });

    // 2. Collect all roomIds
    const roomIds = memberships.map((m) => m.rId);

    // 3. Fetch full room models for each
    const rooms = [];
    for (const roomId of roomIds) {
      try {
        const room = await this._getOneFullRoomModel({ roomId, userId });
        rooms.push(room);
      } catch (e) {
  ...
  async _getOneFullRoomModel(dto: OneFullRoomModel) {
    ...
    let x = await this.roomMemberService.aggregate(
      this.getChannelStagesV3({
        page: 0,
        limit: 1,
    ...
    if (!room)
      throw new BadRequestException(
        "getOneFullRoomModel while aggregate cant find room" +
          dto.roomId +
          " to user" +
          dto.userId
      );
  ...
  async getOrCreatePeerRoom(
    dto: MongoPeerIdDto,
    session?: mongoose.ClientSession
  ) {
    let peerUser: IUser = await this.userService.findByIdOrThrow(
      dto.peerId,
      "userImage fullName fullNameEn"
    );
    dto.peerId = peerUser._id;
    let sRS: ISingleRoomSettings =
      await this.middlewareService.getSingleRoomSetting(
        dto.peerId,
        dto.myUser._id
      );
    if (sRS) {
  ...
  async createOrderRoom(dto: CreateOrderRoomDto) {
    ...
    if (oldRoomSettings) {
      let res = await this.middlewareService.isThereRoomMember(
        oldRoomSettings._id,
        dto.myUser._id
      );
      if (!res)
  ...
  async getRoomById(dto: MongoRoomIdDto) {
    ...
    if (rMember.rT == RoomType.Single) {
      ...
      if (ban)
        throw new BadRequestException("There are ban you dont have access!");
    }
    ...
    if (!rMessage) {
      throw new BadRequestException(
        "WORKING You are try to get room but i dont have messages yet !"
      );
  ...
  async changeNickName(dto: MongoRoomIdDto, name?: string) {
    let rM = await this.middlewareService.isThereRoomMemberOrThrow(
      dto.roomId,
      dto.myUser._id
    );
    if (!name || name == "") {
  ...
  async getMySingleChatInfo(dto: MongoRoomIdDto) {
    let rM: IRoomMember = await this.middlewareService.isThereRoomMemberOrThrow(
      dto.roomId,
      dto.myUser._id
    );
    if (rM.rT != RoomType.Single) {
      throw new BadRequestException(
        "room must be single chat to continue you send " + rM.rT
      );
    }
    let res = {
      banned: false,
    ...
    res["lastSeenAt"] = (
      await this.userService.findById(rM.pId, "lastSeenAt")
    ).lastSeenAt;
    return res;
  }
  ...
  async getMyOrderChatInfo(dto: MongoRoomIdDto) {
    ...
    if (rM.rT != RoomType.Order) {
      throw new BadRequestException(
        "room must be order chat to continue you send " + rM.rT
      );
    }
    let res = {};
  ...
  async getUrlPreview(dto: MongoRoomIdDto, url: string): Promise<any> {
  ...
  async roomOneSeenOn(dto: MongoRoomIdDto) {
    ...
    // Create an info message to notify about the one time seen setting being enabled
    const infoMessageDto = {
      ...
      plm: "web",
      rId: dto.roomId,
      c: "One time seen enabled",
      mT: MessageType.Info,
      msgAtt: {
        adminName: dto.myUser.fullName,
        targetName: "One time seen",
        targetId: dto.myUser._id,
        action: MessageInfoType.OneSeenEnabled,
      },
    ...
    const infoMessage = await this.messageService.createOneFromJson(
      infoMessageDto
    );
  ...
  async roomOneSeenOff(dto: MongoRoomIdDto) {
    ...
    // Create an info message to notify about the one time seen setting being disabled
    const infoMessageDto = {
      sId: dto.myUser._id,
      sName: dto.myUser.fullName,
      sImg: dto.myUser.userImage,
      plm: "web",
      rId: dto.roomId,
      c: "One time seen disabled",
      mT: MessageType.Info,
      msgAtt: {
        adminName: dto.myUser.fullName,
        targetName: "One time seen",
        targetId: dto.myUser._id,
        action: MessageInfoType.OneSeenDisabled,
      },
      lId: require("crypto").randomUUID(),
      isOneSeen: false,
      oneSeenBy: [],
