...
export class RoomMemberService extends BaseRoomService<IRoomMember> {
  ...
  async findCommonRooms(userIds: string[]) {
    const pipeline = [
      ...
      {
        $match: {
          userCount: userIds.length, // All users must be in the room
        },
      },
    ];
  ...
  findAll(
    filter?: FilterQuery<IRoomMember>,
    select?: string,
    options?: QueryOptions<IRoomMember>,
    distinct?: boolean
  ) {
    if (distinct) {
      return Promise.resolve(
        this.model.find(filter, select, options).distinct("rId")
      );
    }
  ...
  findByIdAndUpdate(
    id: string,
    update: Partial<IRoomMember> | null | undefined
  ): Promise<any> {
  ...
  updateMany(
    filter: FilterQuery<IRoomMember>,
    update: Partial<IRoomMember>,
    options?: QueryOptions<IRoomMember> | null | undefined
  ): Promise<any> {
  ...
  async findByIdOrThrow(
    ...
    if (!m)
      throw new NotFoundException(
        "Room member with id " + id + " not found in db"
      );
    return m;
  }
